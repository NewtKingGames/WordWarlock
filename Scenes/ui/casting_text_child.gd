...
func _ready():
	print("TODO add the enlarge text event here")
	rotate_text_start()
	pass
...
func slide_text(direction: int):
	...
	tween_rotate.tween_property(self, "rotation_degrees", rotation * direction, 0.1)
	tween_rotate.tween_property(self, "rotation_degrees", 0, 0.1)
	tween_position.tween_property(self, "position", Vector2(position.x + letter_position_offset*direction, position.y), 0.1)
